function [motion,mov_mat,vid_time_ch]=extract_roi_with_compress_1(path,file)
                global sensitivity
                tic;
                ...
                vid_name1=file_name;
                vid_name2=vid_name1;
                vid_name2=strcat(file(1:end-4),'.avi')
                               
                
                ffmpeg_cmd = [FFMPEG ' -y -i ' vid_name1 ' -r 24 ' vid_name2];
                system(ffmpeg_cmd);  
                copyfile(vid_name2, './temp_data/') 
...
%                 col_m=0;
%                 n = 0;
%                 current_mot=read(obj, 2); 
%                 current_mot=current_mot(:);
%                 prev_mot = current_mot;
%                 
%                 for k = 2 : freq : nframes
%                     img_o=read(obj, k);  
%                     img_o=imresize(img_o,[new_r new_c]);
% %                     img_stdev = std2(img_o);
% %                     col_m=mean(img_o(:))
% %                     col_m = col_m + img_stdev;
% %                     comp_mot_new = abs(currecnt_mot - prev_mot);
%                     
%                     col_m=img_o(:);
%                     current_mot = col_m;
% 
%                     mot_change = corr2(current_mot,prev_mot);
%                     
%                     if mot_change <1
%                         n = n + 1;                        
%                     end
...
%                     
%                 end
%     motion_time_total = (n * freq)/60;
%     disp(['Total time in which there was motion: ' motion_time_total 'minutes']);
%     toc;
